...
<div class="card-header">
    <h5 class="card-title">Solicitud de servicio</h5>
</div>
<style>
.callout td{
...
<div class="card-body">
    <form action="" id="request-form">
        <div class="form-group">
            <label for="email" class="control-label">Enviar servicio a:</label>
            <input type="email" name="email" class='form-control col-md-5' placeholder="Ingresar email(ejemplo@ejemplo.com)" required>
        </div>
...
$(function(){
    $('#request-form').submit(function(e){
        ...
        if($('[name="service[]"]').filter(":checked").length <= 0){
            alert_toast("Selecione mínimo 1 servicio",'error')
            end_loader()
            return false;
        }
        $.ajax({
